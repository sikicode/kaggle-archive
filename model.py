...
MAX_RANSAC_ITERATIONS = 30_000_0
HOMOGRAPHY_CONFIDENCE = 0.99

# DELG model:
SAVED_MODEL_DIR = '../input/pretrained-models/rn101_af_gldv2clean_20200814'
DELG_MODEL = tf.saved_model.load(SAVED_MODEL_DIR)
G2 = tf.saved_model.load('../input/pretrained-models/r101delg_gldv2clean_20200914')
DELG_IMAGE_SCALES_TENSOR = tf.convert_to_tensor([0.70710677, 1.0, 1.4142135])
input_global_scales_ind = tf.range(len(DELG_IMAGE_SCALES_TENSOR))
LOCAL = tf.saved_model.load('../input/pretrained-models/delf_gld_20190411/model')
...
GLOBAL_FEATURE_EXTRACTION_FN = DELG_MODEL.prune(DELG_INPUT_TENSOR_NAMES+['input_global_scales_ind:0'],
                                                ['global_descriptors:0'])

GLOBAL_FEATURE_EXTRACTION_FN2 = G2.prune(DELG_INPUT_TENSOR_NAMES+['input_global_scales_ind:0'],
                                         ['global_descriptors:0'])
# Local feature extraction:
LOCAL_FEATURE_NUM_TENSOR = tf.constant(1000)
LOCAL_FEATURE_EXTRACTION_FN = LOCAL.prune(
...
def extract_global_features(image_root_dir):
    ...
    for i, image_path in enumerate(image_paths):
        ...
        f2 = GLOBAL_FEATURE_EXTRACTION_FN2(image_tensor,
                                           DELG_IMAGE_SCALES_TENSOR,
                                           DELG_SCORE_THRESHOLD_TENSOR,
                                           input_global_scales_ind)
        embeddings[i, :] = tf.nn.l2_normalize(
            tf.reduce_mean(features[0]+2.0*f2[0], axis=0, name='mean'),
            axis=0,
...
def extract_local_features(image_path):
    ...
    keypoints = tf.divide(
        tf.add(
            ...
            tf.gather(features[0], [2, 3], axis=1)), 4.0).numpy()
